...
namespace Minesweeper
{
    public partial class Form_Main : Form
    {
        ...
        int nBGSize = 36; //雷区格子大小
        int nPicSize = 32; //图标大小
        int nBnkSize = 2; //间隔大小
        int nRadius = 8; //圆角矩形半径
        ...
        {
            int nOffsetX = Width - ClientSize.Width; //横向边框宽度
            int nOffsetY = Height - ClientSize.Height; //纵向边框宽度
            int nAdditionY = menuStrip_Main.Height + tableLayoutPanel_Main.Height; //菜单栏及信息栏高度
            Width = nBGSize + (nBGSize + nBnkSize * 2) * nCol + nOffsetX; //窗口宽度
            Height = nBGSize + (nBGSize + nBnkSize * 2) * nRow + nOffsetY + nAdditionY; //窗口高度
        }
        ...
        {
            if (e.X < nBGSize / 2
                || e.X > nBGSize / 2 + nCol * (nBGSize + nBnkSize * 2)
                || e.Y < nBGSize / 2 + menuStrip_Main.Height
                || e.Y > nBGSize / 2 + menuStrip_Main.Height + nRow * (nBGSize + nBnkSize * 2)) //鼠标不在格子内
            {
                pMouseFocus.X = 0;
                pMouseFocus.Y = 0;
            }
        ...
        private void Form_Main_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (bMouseLeft) //松开左键
            {
                ...
                if (nMine[pMouseFocus.Y, pMouseFocus.X] != -1
                ...
                else if (nMine[pMouseFocus.Y, pMouseFocus.X] == -1
                    && nState[pMouseFocus.Y, pMouseFocus.X] != 2) //含地雷 且 未标记为旗 的格子
                {
                    gameLost();
                }
            }
        ...
        {
            ShellAbout(Handle, "扫雷 by rsola in 2015", "一个用C#写的扫雷而已。\n", Icon.Handle);
        }
